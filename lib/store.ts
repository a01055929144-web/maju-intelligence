...
export type StaffRouteDailySummary = {
...
export async function createStaffInvitation(input: StaffInvitationInput, auditContext: AuditActorContext = {}): Promise<{ invitation: StaffInvitation; persisted: boolean }> {
  ...
  await writeAdminAuditLog({
    ...
    metadata: {
      ...
      hasEmployeeEmail: Boolean(invitation.employeeEmail),
...
export async function getCompanyStaffInvitations(companyId: string): Promise<{ invitations: StaffInvitation[]; persisted: boolean }> {
  ...
  const rows = await staffStoreRequest(supabaseRequest<
    Array<{
      ...
      accepted_by: string | null;
      accepted_user: { email: string | null; kakao_user_id: string | null; name: string | null; phone: string | null } | null;
      id: string;
      company_id: string;
      employee_email: string | null;
      employee_name: string | null;
  ...
  >(
    `staff_invitations?select=id,company_id,employee_email,employee_name,employee_phone,invite_code,role,status,expires_at,created_at,accepted_at,accepted_by,accepted_user:app_users!staff_invitations_accepted_by_fkey(email,kakao_user_id,name,phone)&company_id=eq.${encodeURIComponent(
      companyId
...
export async function updateStaffInvitation(input: StaffInvitationUpdateInput, auditContext: AuditActorContext = {}): Promise<{ invitation: StaffInvitation; persisted: boolean }> {
  ...
  if (input.employeeName !== undefined) patch.employee_name = input.employeeName.trim() || "직원";
  if (input.employeePhone !== undefined) patch.employee_phone = input.employeePhone.trim() || null;
  ...
  >(`staff_invitations?select=id,company_id,employee_email,employee_name,employee_phone,invite_code,role,status,expires_at,created_at,accepted_at,accepted_by&id=eq.${encodeURIComponent(input.invitationId)}&company_id=eq.${encodeURIComponent(input.companyId)}`, {
    method: "PATCH",
...
export async function acceptStaffKakaoInvitation(input: StaffKakaoAcceptInput): Promise<StaffKakaoAcceptResult> {
  ...
  >(`staff_invitations?select=id,company_id,employee_email,employee_name,employee_phone,role,status&invite_code=eq.${encodeURIComponent(inviteCode)}&limit=1`));
  ...
  const kakaoEmail = input.email?.trim().toLowerCase() || "";
  const displayName = input.name || invitation.employee_name || "모바일 직원";
  const loginEmail = kakaoEmail || invitation.employee_email?.trim().toLowerCase() || `kakao-${kakaoUserId}@maju.local`;
  const now = new Date().toISOString();
...
export async function acceptStaffOAuthInvitation(input: StaffOAuthAcceptInput): Promise<StaffKakaoAcceptResult> {
  ...
  const company = await getCompanySettings(invitation.company_id);
  const oauthEmail = input.email?.trim().toLowerCase() || "";
  const displayName = input.name || invitation.employee_name || "모바일 직원";
  const loginEmail = oauthEmail || invitation.employee_email?.trim().toLowerCase() || `${input.provider}-${providerUserId}@maju.local`;
  const now = new Date().toISOString();
...
function toStaffInvitation(row: {
  ...
  return {
    acceptedAt: row.accepted_at ? new Date(row.accepted_at).toLocaleString("ko-KR", { timeZone: "Asia/Seoul" }) : undefined,
    acceptedBy: row.accepted_by || undefined,
    acceptedEmail: row.accepted_user?.email || undefined,
    acceptedKakaoUserId: row.accepted_user?.kakao_user_id || undefined,
    acceptedName: row.accepted_user?.name || undefined,
    acceptedPhone: row.accepted_user?.phone || undefined,
    id: row.id,
    companyId: row.company_id,
...
async function upsertStaffRouteDailySummary(input: StaffMobileLocationInput, recordedAt: string) {
  ...
  const previousRows = await supabaseRequest<Array<{ latitude: number | string; longitude: number | string; recorded_at: string }>>(
    `staff_location_events?select=latitude,longitude,recorded_at&company_id=eq.${encodeURIComponent(input.companyId)}&user_id=eq.${encodeURIComponent(
      input.userId
    )}&recorded_at=lt.${encodeURIComponent(recordedAt)}&order=recorded_at.desc&limit=1`
  ).catch(() => []);
  const previous = previousRows[0];
  const previousLat = Number(previous?.latitude);
  const previousLng = Number(previous?.longitude);
  const previousRecordedAt = previous?.recorded_at ? Date.parse(previous.recorded_at) : Number.NaN;
  const currentRecordedAt = Date.parse(recordedAt);
  const distanceKm =
    Number.isFinite(previousLat) && Number.isFinite(previousLng)
      ? Math.min(25, Math.max(0, haversineDistanceKm({ lat: previousLat, lng: previousLng }, { lat: input.lat, lng: input.lng })))
      : 0;
  const drivingMinutes =
    Number.isFinite(previousRecordedAt) && Number.isFinite(currentRecordedAt)
      ? Math.min(60, Math.max(0, Math.round((currentRecordedAt - previousRecordedAt) / 60_000)))
      : 0;
  const visitedCustomerCount = input.currentCustomerId ? 1 : 0;

  const existingRows = await supabaseRequest<
    Array<{
      actual_distance_km: number | string | null;
      driving_minutes: number | string | null;
      estimated_fuel_cost_won: number | string | null;
      estimated_labor_cost_won: number | string | null;
      estimated_total_cost_won: number | string | null;
      first_location_at: string | null;
      id: string;
      location_event_count: number | string | null;
      visited_customer_count: number | string | null;
    }>
  >(
    `staff_route_daily_summaries?select=id,first_location_at,location_event_count,visited_customer_count,actual_distance_km,driving_minutes,estimated_fuel_cost_won,estimated_labor_cost_won,estimated_total_cost_won&company_id=eq.${encodeURIComponent(
      input.companyId
    )}&user_id=eq.${encodeURIComponent(input.userId)}&route_date=eq.${encodeURIComponent(routeDate)}&limit=1`
  ).catch(() => []);
  ...
  const payload = {
    ...
    driver_name: input.driverName || "배송기사",
    driving_minutes: nextDrivingMinutes,
    estimated_fuel_cost_won: fuelCostWon,
    estimated_labor_cost_won: laborCostWon,
    estimated_total_cost_won: fuelCostWon + laborCostWon,
    first_location_at: existing?.first_location_at || recordedAt,
    last_location_at: recordedAt,
    location_event_count: Number(existing?.location_event_count || 0) + 1,
    route_date: routeDate,
    updated_at: recordedAt,
    user_id: input.userId,
    visited_customer_count: Number(existing?.visited_customer_count || 0) + visitedCustomerCount
  };

  await supabaseRequest("staff_route_daily_summaries?on_conflict=company_id,user_id,driver_name,route_date", {
    method: "POST",
    headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
...
export async function upsertStaffMobileLocation(input: StaffMobileLocationInput): Promise<{ location: StaffVehicleLocation | null; persisted: boolean }> {
  ...
  try {
    ...
    if (location) {
      ...
      await upsertStaffRouteDailySummary(input, now);
    }
...
export async function getStaffRouteDailySummaries(
  companyId?: string,
  options: { days?: number; userId?: string } = {}
): Promise<StaffRouteDailySummary[]> {
  if (!companyId || !isProductionStoreConfigured()) return [];
  ...
  try {
    ...
    >(
      `staff_route_daily_summaries?select=id,company_id,user_id,driver_name,delivery_vehicle,route_date,first_location_at,last_location_at,location_event_count,visited_customer_count,actual_distance_km,driving_minutes,estimated_fuel_cost_won,estimated_labor_cost_won,estimated_total_cost_won,cost_basis,updated_at&company_id=eq.${encodeURIComponent(
        companyId
      )}&route_date=gte.${encodeURIComponent(since)}${userFilter}&order=route_date.desc,estimated_total_cost_won.desc&limit=300`
    );
    ...
    return rows.map((row) => ({
      ...
      deliveryVehicle: row.delivery_vehicle || undefined,
      driverName: row.driver_name || "배송기사",
      drivingMinutes: Number(row.driving_minutes || 0),
      estimatedFuelCostWon: Number(row.estimated_fuel_cost_won || 0),
      estimatedLaborCostWon: Number(row.estimated_labor_cost_won || 0),
      estimatedTotalCostWon: Number(row.estimated_total_cost_won || 0),
      firstLocationAt: row.first_location_at || undefined,
      id: row.id,
      lastLocationAt: row.last_location_at || undefined,
      locationEventCount: Number(row.location_event_count || 0),
      routeDate: row.route_date,
      updatedAt: row.updated_at || undefined,
      userId: row.user_id || undefined,
      visitedCustomerCount: Number(row.visited_customer_count || 0)
    }));
  } catch (error) {
    if (error instanceof Error && error.message.includes("staff_route_daily_summaries")) return [];
    throw error;
  }
}
