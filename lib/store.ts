...
export async function enrichPermitLeadExternalInfo(companyId: string, leadId: string): Promise<PermitLeadExternalEnrichmentResult> {
  ...
  {
    // 2026-09-12 보완: 바로 위에서 industry_primary가 "미분류"에서 실제 값으로 막 채워졌더라도
    // (카카오 보강, 이제는 normalizeKakaoCategoryIndustry로 정규화된 값) 여기서 row.industry_primary만
    // 보면 여전히 옛 "미분류"를 봐서 이번 보강분에 업종 인지 점수가 반영되지 않았습니다 — 바로 위
    // phone/address와 같은 패턴으로 payload를 먼저 확인하도록 맞춥니다.
    const industryKnown = ((payload.industry_primary as string | undefined) ?? row.industry_primary ?? "미분류") !== "미분류";
    const scoreBreakdown = computePermitLeadScoreBreakdown({
      leadPeriod: row.lead_period,
