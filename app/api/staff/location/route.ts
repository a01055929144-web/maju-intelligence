...
import { scopeStaffVehicleLocationsForSession, shouldScopeCustomerSession } from "@/lib/customer-data-scope";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const shouldScope = shouldScopeCustomerSession(scope.customerSession);
    const locations = scopeStaffVehicleLocationsForSession(await getStaffVehicleLocations(scope.companyId), scope.customerSession);
    ...
    const events = includeEvents
      ? await getStaffLocationEvents(scope.companyId, {
          ...
          userId: shouldScope ? scope.customerSession?.userId : request.nextUrl.searchParams.get("userId") || undefined
        })
      : undefined;
    ...
    const completions = includeCompletions
      ? await getDeliveryCompletionEvents(scope.companyId, {
          ...
          driverName: shouldScope ? scope.customerSession?.name : request.nextUrl.searchParams.get("driverName") || undefined,
          hours: Number(request.nextUrl.searchParams.get("hours")) || 12
        })
      : undefined;
