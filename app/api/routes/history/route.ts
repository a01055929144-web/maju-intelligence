  ...
  try {
    if (mode === "summary") {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to)) {
        return NextResponse.json({ error: "from, to 날짜(YYYY-MM-DD)가 필요합니다." }, { status: 400 });
      }
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "배송 히스토리를 불러오지 못했습니다." }, { status: 400 });
  }
}
