...
import { deleteDeliveryVehicleFuelType, DeliveryVehicleFuelType, getDeliveryVehicleFuelTypes, upsertDeliveryVehicleFuelType } from "@/lib/store";
...
export async function DELETE(request: NextRequest) {
  const body = (await request.json().catch(() => null)) as { companyId?: string; driverName?: string } | null;
  const scope = await getRequestAuthScope(request, body?.companyId);
  ...
  if (!scopeHasCapability(scope, "manage_customers")) {
    return NextResponse.json({ message: "배송차 정보를 삭제할 권한이 없습니다." }, { status: 403 });
  }
  if (!body?.driverName?.trim()) {
    return NextResponse.json({ message: "담당자명이 필요합니다." }, { status: 400 });
  }
  ...
  try {
    const result = await deleteDeliveryVehicleFuelType(scope.companyId, body.driverName);
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json({ message: error instanceof Error ? error.message : "배송차 삭제에 실패했습니다." }, { status: 400 });
  }
}
