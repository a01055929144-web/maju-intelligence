...
export async function POST(request: NextRequest) {
  ...
  if (typeof body?.deliveryManager !== "string" || !body.deliveryManager.trim()) {
    return NextResponse.json({ message: "담당자명을 입력하세요." }, { status: 400 });
  }

  const result = await bulkUpdateDeliveryManager(scope.companyId, customerIds, body.deliveryManager);
