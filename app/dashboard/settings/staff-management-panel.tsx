...
export function StaffManagementPanel({
  ...
  const [form, setForm] = useState({ employeeEmail: "", employeeName: "", employeePhone: "", role: "driver" as StaffInvitation["role"] });
  const [drafts, setDrafts] = useState<Record<string, { employeeEmail: string; employeeName: string; employeePhone: string }>>({});
  ...
  async function createStaff() {
    ...
    setForm({ employeeEmail: "", employeeName: "", employeePhone: "", role: "driver" });
    setMessage(payload.persisted ? "직원 초대 링크 저장이 완료되었습니다. 링크를 복사해 카카오 가입 안내에 사용하세요." : "직원 초대가 화면에 반영되었습니다. 저장 상태는 시스템 점검에서 확인하세요.");
  }

  async function updateStaff(invitation: StaffInvitation, patch: { employeeEmail?: string; employeeName?: string; employeePhone?: string; role?: StaffInvitation["role"]; status?: "pending" | "revoked" }) {
  ...
  function getDraft(invitation: StaffInvitation) {
    return drafts[invitation.id] || {
  ...
  function updateDraft(invitation: StaffInvitation, patch: Partial<{ employeeEmail: string; employeeName: string; employeePhone: string }>) {
    setDrafts((current) => ({ ...current, [invitation.id]: { ...getDraft(invitation), ...patch } }));
  }
            ...
            <div key={invitation.id} className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate text-xs font-bold text-slate-500">
                    {[invitation.employeeEmail, invitation.employeePhone].filter(Boolean).join(" · ") || "매핑값 미입력"}
                  </p>
                </div>
              ...
              <div className="mt-4 grid gap-2 md:grid-cols-3">
                <input
                  ...
                  onChange={(event) => updateDraft(invitation, { employeeName: event.target.value })}
                  placeholder="직원명"
                  value={getDraft(invitation).employeeName}
                />
                <input
                  className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold outline-none transition focus:border-teal-400 focus:ring-2 focus:ring-teal-100 disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={!canManageMembers || savingId === invitation.id}
                  onChange={(event) => updateDraft(invitation, { employeeEmail: event.target.value })}
                  placeholder="이메일"
                  value={getDraft(invitation).employeeEmail}
                />
                <input
                  className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold outline-none transition focus:border-teal-400 focus:ring-2 focus:ring-teal-100 disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={!canManageMembers || savingId === invitation.id}
                  onChange={(event) => updateDraft(invitation, { employeePhone: event.target.value })}
                  placeholder="연락처"
                  value={getDraft(invitation).employeePhone}
                />
              </div>
              ...
              <div className="mt-2 grid gap-2 md:grid-cols-[1fr_auto_auto_auto]">
                ...
                <Button disabled={!canManageMembers || savingId === invitation.id} onClick={() => updateStaff(invitation, getDraft(invitation))} type="button" variant="outline">
                  저장
                </Button>
                <Button
                  disabled={!canManageMembers || savingId === invitation.id}
              ...
              {invitation.acceptedBy ? (
                <p className="mt-3 truncate rounded-md bg-emerald-50 px-3 py-2 text-[11px] font-bold text-emerald-800">
                  연결됨 · {[invitation.acceptedName, invitation.acceptedKakaoUserId ? `Kakao ${invitation.acceptedKakaoUserId}` : "", invitation.acceptedEmail, invitation.acceptedPhone].filter(Boolean).join(" · ") || invitation.acceptedBy}
                  {invitation.acceptedAt ? ` · ${invitation.acceptedAt}` : ""}
                </p>
              ) : null}
          ...
          {canManageMembers ? (
            <div className="mt-4 grid gap-3">
              ...
              <input
                ...
                onChange={(event) => setForm((prev) => ({ ...prev, employeeEmail: event.target.value }))}
              />
              <input
                className="h-11 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold outline-none transition focus:border-teal-400 focus:ring-2 focus:ring-teal-100"
                placeholder="연락처"
