              ...
              <Card className="border-slate-200/80 shadow-sm">
                <CardHeader className="pb-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-muted-foreground">점수, 예상매출, 지역을 기준으로 오늘 볼 영업 후보입니다.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {primaryLead ? <Badge className="bg-accent/20 text-foreground">1순위 {primaryLead.region}</Badge> : null}
                      <Link className="inline-flex h-9 items-center gap-2 rounded-md bg-slate-950 px-3 text-xs font-black text-white transition hover:bg-slate-800" href={withCompanyQuery("/revenue/pipeline")}>
                        파이프라인 열기
                        <ArrowRight className="h-3.5 w-3.5" />
                      </Link>
                    </div>
                ...
                <CardContent className="grid gap-3 lg:grid-cols-2">
                  {topLeads.map((lead, index) => (
                    <div key={lead.id || lead.name} className="relative overflow-hidden rounded-md border border-slate-200 bg-white p-3 shadow-[0_1px_0_rgba(15,23,42,0.03)]">
                      <span className="absolute inset-x-0 top-0 h-1 bg-blue-600" />
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="flex min-w-0 gap-3">
                          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-blue-700 text-xs font-black text-white">{index + 1}</span>
                          <div className="min-w-0">
                            <p className="truncate text-sm font-black text-slate-950">{lead.name}</p>
                            <p className="mt-1 truncate text-xs font-bold text-slate-500">{lead.region}</p>
                          </div>
                        </div>
                        <Badge className="bg-blue-50 text-blue-700">{lead.score}점</Badge>
                      </div>
                      <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_124px] sm:items-end">
                        <div className="rounded-md bg-slate-50 px-3 py-2">
                          <p className="text-[11px] font-black uppercase tracking-wide text-slate-400">예상 월매출</p>
                          <p className="mt-1 text-base font-black text-slate-950">{lead.expectedRevenue.toLocaleString()}만원</p>
                        </div>
            ...
            <Card className="border-slate-200/80 shadow-sm">
              <CardHeader className="pb-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  ...
                  <Badge className="w-fit bg-teal-50 text-teal-700">1 데이터 → 2 코스 → 3 매출 → 4 도우미</Badge>
                </div>
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                {quickActions.map((action, index) => (
                  <QuickActionCard key={action.href} step={index + 1} {...action} />
                ))}
              </CardContent>
            </Card>
          }
...
function QuickActionCard({
  ...
  return (
    <Link className="group relative overflow-hidden rounded-md border border-slate-200/80 bg-white p-4 shadow-[0_1px_0_rgba(15,23,42,0.03)] transition hover:border-teal-200 hover:bg-teal-50/50" href={href}>
      <span className="absolute inset-x-0 top-0 h-1 bg-teal-600" />
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="grid h-9 w-9 place-items-center rounded-md bg-slate-950 text-xs font-black text-white">{step}</span>
          <div className="flex h-9 w-9 items-center justify-center rounded-md bg-slate-100 text-slate-600 group-hover:bg-white group-hover:text-teal-700">
            <Icon className="h-4 w-4" />
          </div>
        </div>
        <ArrowRight className="h-4 w-4 text-slate-400 transition group-hover:translate-x-0.5" />
