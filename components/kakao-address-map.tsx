...
export type KakaoMovingVehicle = {
...
type KakaoAddressMapProps = {
  ...
  readonly movingVehicles?: ReadonlyArray<KakaoMovingVehicle>;
...
export function KakaoAddressMap({
  ...
  const movingVehicleOverlaysRef = useRef<any[]>([]);
  const movingVehicleTimerRef = useRef<number | null>(null);
  ...
  const markersSignature = useMemo(
    () => markers.map((marker) => `${marker.id || ""}|${marker.name || ""}|${marker.address || ""}|${marker.lat || ""}|${marker.lng || ""}|${marker.tone || ""}`).join(";;"),
    [markers]
  );
  const movingVehicleSignature = useMemo(
    () =>
      movingVehicles
        .map((vehicle) => `${vehicle.id}|${vehicle.label}|${vehicle.name}|${vehicle.routePath?.length || 0}`)
        .join(";;"),
    [movingVehicles]
  );
  ...
  useEffect(() => {
    ...
    async function bootMap() {
      ...
      try {
        ...
        await Promise.all(
          markers.map((marker) => {
            if (Number.isFinite(marker.lat) && Number.isFinite(marker.lng)) {
              if (!ignore) placeMarkerAtPosition(marker, Number(marker.lat), Number(marker.lng));
        ...
        window.setTimeout(() => {
          ...
          drawMovingVehicleOverlays(kakao, map, movingVehicles, movingVehicleOverlaysRef, movingVehicleTimerRef);
        }, 120);
      } catch (error) {
        if (!ignore) {
    ...
    return () => {
      ignore = true;
      clearMovingVehicleOverlays(movingVehicleOverlaysRef, movingVehicleTimerRef);
    };
  ...
  }, [appKey, canUseKakao, markersSignature, movingVehicleSignature, routePath]);
...
function drawMovingVehicleOverlays(
  kakao: any,
  map: any,
  vehicles: ReadonlyArray<KakaoMovingVehicle>,
  overlayRef: { current: any[] },
  timerRef: { current: number | null }
) {
  clearMovingVehicleOverlays(overlayRef, timerRef);
  const drawableVehicles = vehicles
    .map((vehicle) => {
      const path = (vehicle.routePath || [])
  ...
  if (!drawableVehicles.length) return;

  const overlays = drawableVehicles.map(({ path, vehicle }, index) => {
    const overlay = new kakao.maps.CustomOverlay({
      content: createMovingVehicleOverlay(vehicle),
      map,
      position: path[0],
      yAnchor: 1.2,
      zIndex: 60 + index
    });
    return { index, overlay, path, vehicle };
  });
  overlayRef.current = overlays.map((entry) => entry.overlay);

  let frame = 0;
  timerRef.current = window.setInterval(() => {
    frame += 1;
    overlays.forEach((entry) => {
      const position = interpolateLatLng(kakao, entry.path, (frame + entry.index * 18) / 90);
      entry.overlay.setPosition(position);
    });
  }, 90);
}

function clearMovingVehicleOverlays(overlayRef: { current: any[] }, timerRef: { current: number | null }) {
...
function interpolateLatLng(kakao: any, path: any[], progress: number) {
  if (path.length < 2) return path[0];
  const segmentCount = path.length - 1;
  const wrapped = progress % segmentCount;
...
function createMovingVehicleOverlay(vehicle: KakaoMovingVehicle) {
  const color = escapeHtml(vehicle.color || "#0f766e");
  const label = escapeHtml(vehicle.label || "차");
  const name = escapeHtml(vehicle.name || "배송차");
  return htmlToElement(`
    <div title="${name} 운행 중" style="position:relative;display:flex;align-items:center;gap:6px;transform:translateY(-4px);pointer-events:none;">
      <span style="position:absolute;left:50%;top:50%;width:42px;height:42px;margin-left:-21px;margin-top:-21px;border-radius:999px;background:${color};opacity:.14;animation:majuVehiclePulse 1.5s ease-out infinite;"></span>
      <span style="position:relative;display:grid;place-items:center;width:34px;height:34px;border-radius:999px;background:${color};color:#ffffff;border:3px solid #ffffff;box-shadow:0 12px 24px rgba(15,23,42,.3);font-size:11px;font-weight:900;">${label}</span>
      <span style="position:relative;border:1px solid rgba(15,23,42,.12);border-radius:999px;background:#ffffff;color:#0f172a;padding:5px 8px;font-size:11px;font-weight:900;box-shadow:0 8px 18px rgba(15,23,42,.18);white-space:nowrap;">운행중 · ${name}</span>
    </div>
  `);
}
...
function createMarkerOverlay(marker: KakaoMapMarker, compactLead = false) {
  ...
  if (marker.tone === "vehicle") {
    return htmlToElement(`
      <button type="button" title="${name}" style="cursor:pointer;background:#0f766e;color:#ffffff;border:2px solid #ffffff;border-radius:999px;display:flex;align-items:center;gap:6px;padding:7px 10px;box-shadow:0 10px 24px rgba(15,118,110,.32);font-size:11px;font-weight:900;white-space:nowrap;">
        <span style="width:8px;height:8px;border-radius:999px;background:#5eead4;box-shadow:0 0 0 4px rgba(94,234,212,.22);display:block;"></span>
        차 ${label}
      </button>
    `);
  }
      ...
      {markers.map((marker) => (
        <div key={`${marker.label}-${marker.name}`} className="rounded-md border border-border bg-white p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              {marker.grade ? `${marker.grade}등급` : marker.tone === "origin" ? "출발지" : marker.tone === "lead" ? "신규" : marker.tone === "vehicle" ? "차량" : "거래처"}
            </Badge>
          </div>
