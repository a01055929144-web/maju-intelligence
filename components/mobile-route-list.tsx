...
export function MobileRouteList({ completedCustomerIds = [], driverName, initialStops, routeArea, selectedStopId }: { completedCustomerIds?: string[]; driverName: string; initialStops: MobileRouteStop[]; routeArea: string; selectedStopId?: string }) {
  ...
  async function endDrag(event: React.PointerEvent<HTMLButtonElement>) {
    ...
    try {
      ...
      if (response.ok) { lastSavedStopsRef.current = pendingStops; setMessage("변경한 방문 순서를 저장했습니다."); }
      else rollback(payload?.message || "순서 저장에 실패해 이전 순서로 되돌렸습니다.");
    } catch { rollback("네트워크 오류로 순서를 저장하지 못해 이전 순서로 되돌렸습니다."); }
  ...
  return <section className="mobile-card scroll-mt-24 rounded-2xl border p-4 shadow-sm" id="route-list">
    <div className="flex items-center justify-between gap-3"><div><p className="mobile-accent text-xs font-black">오늘의 경로</p><p className="mt-1 text-lg font-black">{completedCount} / {stops.length}곳 완료</p></div><span className="mobile-accent-soft rounded-full px-3 py-1.5 text-xs font-black">진행 {Math.max(0, activeIndex + 1)}</span></div>
    ...
    <div className="mt-4 flex items-start overflow-hidden pb-2">{stops.slice(Math.max(0, activeIndex - 2), activeIndex + 3).map((stop, visibleIndex) => { const index = stops.findIndex((item) => item.id === stop.id); const done = completed.has(stop.id); const active = stop.id === selectedStopId; return <div className="flex min-w-0 flex-1 items-start" key={stop.id}><div className="min-w-0 flex-1 text-center"><span className={`mx-auto grid h-8 w-8 place-items-center rounded-full text-xs font-black ${done ? "bg-teal-600 text-white" : active ? "bg-slate-950 text-white ring-4 ring-teal-100" : "bg-slate-100 text-slate-500"}`}>{done ? <CheckCircle2 className="h-4 w-4" /> : index + 1}</span><p className={`mt-1 truncate text-[10px] font-black ${active ? "text-teal-300" : "text-slate-400"}`}>{stop.name}</p></div>{visibleIndex < Math.min(5, stops.length) - 1 ? <span className={`mt-4 h-0.5 w-3 shrink-0 ${index < activeIndex || done ? "bg-teal-500" : "bg-slate-600"}`} /> : null}</div>; })}</div>
    <button className="mobile-card-raised mt-2 flex h-11 w-full items-center justify-between rounded-xl border px-3 text-sm font-black" onClick={() => setSheetOpen(true)} type="button"><span className="flex items-center gap-2"><Truck className="mobile-accent h-4 w-4" />전체 목록 보기</span><span className="mobile-accent">({stops.length})</span></button>
    {sheetOpen ? <div aria-modal="true" className="fixed inset-0 z-50 flex items-end bg-slate-950/70" role="dialog" onClick={() => setSheetOpen(false)}><div className="max-h-[86dvh] w-full rounded-t-3xl border border-slate-700 bg-[#111827] text-white shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="mx-auto mt-2 h-1.5 w-12 rounded-full bg-slate-600" /><div className="flex items-center justify-between border-b border-slate-700 px-4 py-3"><div><p className="font-black">오늘의 전체 경로</p><p className="text-xs font-bold text-slate-400">{driverName} · {routeArea} · 대기 매장은 길게 눌러 이동</p></div><button aria-label="닫기" className="grid h-10 w-10 place-items-center rounded-full bg-slate-800" onClick={() => setSheetOpen(false)} type="button"><X className="h-5 w-5" /></button></div><div className="max-h-[calc(86dvh-78px)] divide-y divide-slate-800 overflow-y-auto pb-[calc(1rem+env(safe-area-inset-bottom))]">
      ...
      {!stops.length ? <p className="p-6 text-center text-sm font-bold text-slate-500">오늘 배정된 코스가 없습니다.</p> : null}{message ? <p aria-live="polite" className={`sticky bottom-0 bg-white px-4 py-3 text-xs font-bold shadow-[0_-4px_12px_rgba(15,23,42,.06)] ${saveError ? "text-rose-700" : "text-teal-700"}`}>{saving ? "순서를 저장 중입니다." : message}</p> : null}
    </div></div></div> : null}
  </section>;
}

function Small({ icon: Icon, label }: { icon: typeof MapPinned; label: string }) { return <span className="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-black text-slate-600 ring-1 ring-inset ring-slate-100"><Icon className="h-3 w-3" />{label}</span>; }
