...
import { Clock, GripVertical, MapPinned, Phone, Truck } from "lucide-react";
...
export function MobileRouteList({ driverName, initialStops, routeArea, selectedStopId }: {
  driverName: string;
  ...
  async function endDrag() {
    ...
    try {
      const response = await fetchWithTimeout("/api/routes/confirm-order", {
        ...
        body: JSON.stringify({ driverName, customerIds: stopsRef.current.map((stop) => stop.id) })
      }, 15000);
      const payload = await response.json().catch(() => null) as { message?: string } | null;
      setMessage(response.ok ? "변경한 방문 순서를 저장했습니다." : payload?.message || "순서 저장에 실패했습니다.");
    } catch {
      setMessage("네트워크 오류로 순서를 저장하지 못했습니다.");
    } finally {
      setSaving(false);
    }
  }
        ...
        {stops.map((stop, index) => (
          <div className={`relative flex items-start gap-3 p-4 ${selectedStopId === stop.id ? "bg-teal-50/70" : ""}`} data-route-stop-id={stop.id} key={stop.id}>
            ...
            <div className="relative z-10 min-w-0 flex-1 pointer-events-none">
              <div className="flex items-center gap-2"><p className="truncate font-black">{stop.name}</p><Badge className="shrink-0 bg-slate-100 text-slate-700">{stop.region}</Badge></div>
            ...
            <button aria-label={`${stop.name} 순서 이동`} className="relative z-20 grid h-10 w-10 touch-none place-items-center rounded-lg border border-slate-200 text-slate-500" disabled={saving} onPointerCancel={endDrag} onPointerDown={(event) => beginDrag(stop.id, event)} onPointerMove={moveDrag} onPointerUp={endDrag} type="button"><GripVertical className="h-5 w-5" /></button>
          </div>
        ))}
        {!stops.length ? <div className="p-4 text-sm font-bold text-slate-500">오늘 배정된 코스가 없습니다.</div> : null}
      </div>
      {message ? <p aria-live="polite" className="border-t border-slate-100 px-4 py-3 text-xs font-bold text-teal-700">{saving ? "순서를 저장 중입니다." : message}</p> : null}
    </section>
  );
}
