...
- 2026-09-21: 지도 홈 초기 거래처 조회를 `getCustomerMapSummaries`로 경량화했습니다. 이름·주소·매출등급·담당자/차량 매칭 필드만 먼저 읽고, 사업자·첨부·리뷰·출입정보 등 원장 상세 컬럼은 지도 첫 로드에서 제외합니다.
- 2026-09-23: 오늘 배송 코스를 담당자별 확정 목록으로 분리했습니다. 선택하지 않은 거래처는 당일 `배송 없음`으로 제외되고 다음 날 다시 후보가 되며, 2곳 이상 코스는 티맵 루트 최적화 후에만 확정됩니다.
- 2026-09-23: 차량 분석의 고정 12시간 제한을 날짜 선택 조회로 교체했습니다. 운행 기록은 삭제되지 않으며 차량 팝업의 날짜 필터 또는 히스토리 달력에서 과거 GPS·완료 기록을 계속 조회합니다.
- 2026-09-23: 배송완료 지도 마커의 체크 배지를 확대하고, 히스토리에서 확정 코스 대비 완료·대기·과거 미완료 거래처를 구분합니다.
- 2026-09-23: 지도 홈의 담당자·차량 패널에 오늘 배송 예정·완료·대기 매장 수와 배송 없음 담당자 수를 한눈에 보는 집계 및 담당자별 배지를 추가했습니다.
- 2026-09-23: 과거 확정 코스의 미완료 매장에 `재배송`·`취소`·`확인` 후속 처리를 추가했습니다. `delivery_route_follow_ups`에 회사·날짜·거래처 기준으로 영구 저장하며 처리자·역할·처리시각을 함께 남깁니다. 재배송 대상도 다음 날 기존 전체 후보 복귀 정책을 그대로 따릅니다. 운영 반영 전 `20260923_delivery_route_follow_ups.sql` 적용이 필요합니다.
...
- 대표·관리자는 지도 좌측 `담당자 · 차량` 패널의 `차량 마스터`에서 차량을 등록하고 차량명·번호·연료·메모를 수정하거나 상태를 바꿀 수 있습니다. 일반 직원은 조회만 가능합니다.
