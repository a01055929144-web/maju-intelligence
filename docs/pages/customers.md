...
## Task
**(2026-09-13 구현 완료) 첨부파일 종류별 누락 경고 강화**
- 거래처 첨부 패널에서 사업자등록증은 `필수 누락`, 통장사본·배송 적재위치는 `운영 보완 권장`으로 구분한다.
- 첨부 조회 실패를 0건으로 오인하지 않도록 별도 오류와 `다시 확인` 액션을 표시한다.
- 모든 필수·권장 자료가 연결되면 완료 상태를 명시한다.
...
## LAST VERIFIED
2026-09-19 — 거래처 0건 오류 구분과 교차 중복 정리 변경을 포함해 전체 검증 완료.
...
- [x] 다중 첨부 업로드의 성공 건은 유지하고 실패 건만 즉시 다시 시도하는 복구 액션 추가
