...
## COMPLETED
- [x] 360/390/430px 기사 화면에서 지도·전화·주소 버튼 높이와 간격을 반응형으로 압축
- [x] 선택 매장 완료 상태를 명확히 표시하고, 저장 후 다음 미완료 매장(마지막 순번에서는 앞쪽 미완료 포함)으로 자동 이동
- [x] GPS 전송 대기 건수와 로컬 보관 상태, 부분 저장 실패 시 실패 단계만 재시도됨을 버튼 가까이에 안내
- [x] 순서 저장 실패 시 직전 변경 순서를 보관해 재시도하고, 직접 매장 선택 대안을 함께 제공
- [x] 오프라인·재접속·위치 권한 거부 상태를 구분해 기사에게 다음 행동을 안내
- [x] 오늘 배송 없음으로 제외된 매장이 다음 운영일 배정 후보로 복귀함을 빈 코스에 안내
...
- [x] 현장 화면 본문을 `오늘 코스 → 선택 매장 → 지도·전화 → 적재위치 → 배송완료·사진 → 메모` 순서로 정렬
- [x] 담당 직원에게 회사 전체 코스 건수가 폴백 노출되지 않도록 방문처·코스 요약을 담당 범위로 제한
- [x] 기사 첫 진입 시 오늘 코스를 기본으로 열고, 매장 선택 후에만 선택 매장 단계를 자동 오픈
